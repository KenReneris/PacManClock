...
NtpClient::Setup()
{
    ...
    g_globalState.EnableState( GlobalState::TimeNotSet );
}
...
NtpClient::CheckForResponse()
{
    ...
    if ( cb != NTP_PACKET_SIZE ) 
    {
        if ( g_poweredOnTime > _nextTime )
        {
            Marquee::Show( F("Ntp: response timeout\n") );
            SetState( WaitingForRetry );
        }
    }
    else
    {
        ...
        g_globalState.ClearState( GlobalState::TimeNotSet );
